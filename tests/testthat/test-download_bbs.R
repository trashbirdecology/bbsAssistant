...
test_that("Expect at least 2 .zip files to be downloaded via FTP",{
testthat::skip_on_travis()
              ...
              # create dir...
              suppressMessages(dir.create(tempdir))
              
              # download two state files
              download_bbs(data.dir = tempdir, state.names = c("FloriDa", "NebrASKA"))
              files <- list.files(tempdir)
              expect_true(length(files)==2, label = "Number of downloaded state files using function download_bbs does not equal 2. Please check to see if PWRC updated the files...")
