...
```{r setup}
devtools::install_github("trashbirdecology/bbsAssistant", "main", force=TRUE)
...
```{r dataavail}
data(bbs_obs, package="bbsAssistant")
data(bbs_obs, package="bbsAssistant")
```
...
rm(bbs_data) # clear list, unless you want it still or need the weather...
```
