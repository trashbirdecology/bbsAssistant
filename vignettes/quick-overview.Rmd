...
## A: Retrieving, Downloading, and Manipulating the route-level data
This package was creating for expediting the BBS data retrieval process, which currently comprises multiple .zip files retrievable individually via FTP server, and caters to those wishing to access BBS route-level data across _multiple states_. 

### Download the BBS data from USGS
First, please specify a direcotry within which to store the downloaded BBS data. Here, we specify the directory as `/raw-data/`, and create the directory if it does not already exist. 

```{r get-bbs-data1}
bbsDir <- here::here("raw-data/")
dir.create(bbsDir) 
```

Next, we can use the function, `get_bbsData` to retrieve all or a selection of the data from the USGS webpage using FTP. These compressed (.zip) files are stored to local directory, as specified by argument `data.dir`. Let's download only the data from the U.S. (country code 840) for years 2000-2016:

```{r get-bbs-data2, message=FALSE}
download_bbs(year = 2000:2016, country.nums = 840, data.dir = bbsDir)
```

### Import the BBS state-level data into R for manipulation
Now, let's import the data into R by first decompressing the .zip files in `~/raw-data`, and loading into file as a single data frame. 
