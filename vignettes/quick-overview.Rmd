...
# This data frame differs slightly from BBS raw data in that we have appended State Names (`State`) to the df
bbs.df$State %>% unique()
```

### Download and route-level information
...
```{r}
conditions <- get_conditions(data.dir=bbsDir)
```
...
Results will be updated annually as they are released from the Patuxent Wildlife Research Centerm and can be called using the function `utils::data()`:
