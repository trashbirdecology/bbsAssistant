...
date: "`r Sys.Date()`"
output: # pdf_document # when rendering PDF silence badges .svg + md_document
  md_document:
...
# Function Manual
For function descriptions and examples please see the [manual](https://github.com/trashbirdecology/bbsassistant/bbsAssistant_0.0.1.pdf). 

# Contributions
If you would like to contribute, please submit a pr, create an issue, or email us (jessicaleighburnett at gmail). Contributors must adhere to the [Code of Conduct](CODE_OF_CONDUCT.MD). 
...
## First install the development version of the package. If updates have not occurred, using `force=FALSE` will not fetch the pkg
devtools::install_github("trashbirdecology/bbsAssistant", force=TRUE, dependencies = TRUE)
...
First, let's retrieve the regions of data that are available. The function `get_regions` retrieves the .zip filenames of all U.S. states and Canadian provinces, including their reference numbers and region codes.
...
### Create a series of filenames for states, regions
...
### Limit the data downloaded, since downloading all regions may take up too much space. The state or province names are truncated, so in many cases we don't want to specify the entire state name (e.g., 'Florida.zip')
regionFileName.use <- regionFileName[which(str_detect(regionFileName, "Flori")==TRUE)]

### Input multiple regions
multRegionsFileName.use <- regionFileName[which(str_detect(regionFileName, paste(c("Flori", "Neb", "Jersey"), collapse = '|')))]
...
Alternatively, import data files you specified with the function regionFileName.use  Use the same code as above to import *multiple* feathers from file:
```{r loadAllData, eval = FALSE, echo=TRUE}
## In this example we only used Florida
feathers <- NULL
featherNames <- list.files(bbsDir, pattern = ".feather")
featherNames <- str_c("/", featherNames) #add separator
...
We can sort by AOU # (e.g. House Sparrow aou = 06882) within a dataset, in this case we are using the Florida BBS data. 
...
# Remove a single family, Passeridae
flBBS_sub1 <- subset_speciesList(flBBS, fam.ind = "Passeridae") 
unique(flBBS_sub1$family) # notice Passeridae is not in the list, only 54 families

# Or remove multiple families
flBBS_sub2 <- subset_speciesList(flBBS, fam.ind = c("Passeridae", "Parulidae")) 
unique(flBBS_sub2$family) # notice Passeridae and Parulidae are not in the list, only 53 families
```
