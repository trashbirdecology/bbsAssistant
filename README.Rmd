...
## First install bbsAssistant using devtools 
...
# b. Create a series of filenames for states, regions
regionFileName <- regions$zipFileName %>% na.omit()

# c. Let's limit the data downloaded, since downloading all regions may take up too much space. The state or province names are truncated, so in many cases we don't want to specify the entire state name (e.g., 'Florida.zip')
...
```
Or import data files you specified with the function regionFileName.use 
...
unique(flBBS$family) # notice 55 families in total
