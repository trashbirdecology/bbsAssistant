...
hosp.aou.code <- bbs$species_list$AOU[bbs$species_list$Genus=="Passer"& bbs$species_list$Species=="domesticus"]

# filter the observations along  BBS's "AOU" code:
test=bbs$observations
test %>% filter(AOU == hosp.aou.code)
bbs$observations %>% as.data.frame() %>% filter(AOU %in% hosp.aou.code)
```
