...
get_regions <-
    function(source = "ftp://ftpext.usgs.gov/pub/er/md/laurel/BBS/DataFiles/",
        ...
        # Read in the codes
        CountryCodes <-
            readr::read_table(File, skip = 4,  col_names = F)[1:3,]
        ...
        zipf = readme.all %>%
            ...
            dplyr::mutate(zipFileName =
                       stringr::word(V1, 1)) %>%
            dplyr::mutate(stateName = stringr::str_squish(toupper(stringr::word(V1, 2:3, -1)))) %>%
            dplyr::select(zipFileName, stateName)
        
        RegionCodes <-
            dplyr::right_join(RegionCodes %>%  dplyr::filter(countryNum != 484) %>%
                                  dplyr::mutate(stateName = toupper(stateName)),
                       zipf)
