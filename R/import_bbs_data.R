...
state.fns.ind <- list.files(paste0(sb_dir, "/States"), pattern=".csv", full.names=FALSE) # purely for use in status messages.
...
for(i in seq_along(state.fns)){
    message("Importing observations data:  ", state.fns.ind[i], " (File ",i," of ", length(state.fns), ")")
    suppressMessages( temp <- readr::read_csv(state.fns[i])) # suppress all the messages about column types...bleh need to resolve this
    temp <- temp %>% 
