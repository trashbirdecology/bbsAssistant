...
##i know there's a way to clean up with filter_Across or at or something but w/ev for now...
  ...
  bbs_list$observations <- bbs_list$observations %>% filter(AOU %in% aous.keep)
...
bbs_list$observations <-
  bbs_list$observations %>%
  mutate(RouteTotal=rowSums(across(all_of(cols.stop))))
...
bbs_list$vehicle_data <- bbs_list$vehicle_data %>%
  mutate(WindMean= round(rowSums(across(all_of(cols.car)))/50)) # rounded
...
if(observations.output!="list"){
  cat("Collapsing BBS data and metadata into a single data frame.\n")
    df <- left_join(bbs_list$observations, bbs_list$vehicle_data)
    df <- left_join(df, bbs_list$routes)
    df <- left_join(df, bbs_list$metadata)
    df <- left_join(df, bbs_list$weather)
    bbs_list <- df
    rm(df)
}
